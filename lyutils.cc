...
int yylval_token (int symbol) {
   ...
   yylval = new_astree (symbol, included_filenames.size() - 1,
                        scan_linenr, offset, yytext/*,NULL*/ );
   fprintf(tokfile, "%lu %d.%03d %d %s ", 
                        included_filenames.size() - 1,
                        scan_linenr, scan_offset,
...
astree* new_parseroot (void) {
  yyparse_astree = new_astree (TOK_ROOT, 0, 0, 0, ""/*, NULL*/);
   //printf("test\n");
...
RCSC("$Id: lyutils.cc,v 1.6 2014-12-01 16:34:53-08 - - $")
